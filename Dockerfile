# Use Dockerimage for Node 16
FROM node:16 as ghost_builder

# copy the ghost folder into the container
COPY . /Ghost
...
COPY --from=ghost_builder /Ghost/ghost-*.tgz /tmp/ghost.tgz
...
RUN set -eux; \
	...
	aptPurge=; \
	\
	installCmd='gosu node ghost install --archive /tmp/ghost.tgz --db mysql --dbhost mysql --no-prompt --no-stack --no-setup --dir "$GHOST_INSTALL"'; \
